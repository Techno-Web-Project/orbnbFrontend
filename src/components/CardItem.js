  ...
  return (
    <>
      <ul className="cards__item">
        {houses.filter((house)=> {
          if((props.searchedCity2 == "" || props.searchedCity2 == "...") && props.searchedBeds2 == 1) {
            return house
          } else if ((house.city.toLowerCase().includes(props.searchedCity2.toLowerCase()) || props.searchedCity2 == "...") && house.numberOfBed >= props.searchedBeds2) {
            return house
          }
        }).map(house => (
          <li key={house.id} className="cards__individuelle">
              <Link className="cards__item__link" to={props.path}>
                  <figure className="cards__item__pic-wrap" data-category={props.label}>
                    <img src={`images/${house.housingPictures[0].fileLocalisation}`} alt="Travel"
